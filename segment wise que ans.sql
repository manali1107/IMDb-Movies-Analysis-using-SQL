...
/* ques:1 Q1. What are the different tables in the database and how are they connected to each other in the database?
 Ans:- Within the database, there are six tables:
1. movie: This particular table holds data regarding movies. It possesses a primary key labeled as `id` and includes columns such as `title`, `year`, `date_published`, 
`duration`, `country`, `worlwide_gross_income`, `languages`, and `production_company`.
2. genre: This table signifies the different genres of movies. It includes a composite primary key `(movie_id, genre)` and stores the movie ID along with its associated genre. 
3. The director_mapping table associates movies with directors. It has a primary key composed of the movie ID and director name, and includes the movie ID 
and director name for each movie.
4. The role_mapping table associates movies with actors/actresses and their roles. It has a primary key composed of the movie ID and actor/actress name, 
and includes the movie ID, actor/actress name, and role category for each movie.
5. The names table holds data regarding individuals who have participated in movies, including actors, actresses, and directors. It consists of a primary key,
 "id," and features like "name," "height," "date_of_birth," and "known_for_movies."
6. The ratings table encompasses information about the ratings of movies. It incorporates a primary key, "movie_id," and attributes such as
 "avg_rating," "total_votes," and "median_rating."
Using foreign keys, these tables establish connections among each other. In the genre, director_mapping, and role_mapping tables, the `movie_id` column refers to
 the `id` column in the movie table. Likewise, in the director_mapping and role_mapping tables, the `name_id` column points to the `id` column in the names table. 
 Thanks to these relationships, it becomes possible to associate movies with genres, directors, and actors/actresses. */
 
 -- ques 2: Q2. Find the total number of rows in each table of the schema.
